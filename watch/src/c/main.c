...
static void prv_window_load(Window *window) {
  ...
  // Both faces are system fonts, owned by the firmware. They are taken by key
  // and never unloaded -- fonts_unload_custom_font() is only for fonts the app
  // itself loaded from its own resources, and this one has none.
...
static void prv_window_unload(Window *window) {
  ...
  s_canvas = NULL;
  // Nothing to unload: both fonts belong to the firmware.
}
