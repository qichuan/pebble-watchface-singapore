...
- App icons at 80px and 144px, and a 25px menu icon for the watch's launcher,
  all generated by `tools/make_icons.py` from the same construction the face
  uses. The icons carry a larger crescent and larger stars than the face does,
  since at icon size the emblem is the only part of the flag that identifies it.
  The menu icon is the flag in two tones — the emblem in black on a white
  ground, over a black band — sized so the five points survive the 1-bit
  conversion the SDK applies to it.
